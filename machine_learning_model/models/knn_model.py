...
class KNNModel:
    ...
    def __init__(self, random_state: int = 42) -> None:
        ...
        self.neighbor_analysis: Optional[Dict[str, Any]] = None
    ...
    def find_optimal_k(self, X_train: np.ndarray, y_train: np.ndarray, k_range: Optional[List[int]] = None, cv: int = 5, scoring: str = 'f1_macro') -> int:
        """
    ...
    def train(self, X_train: np.ndarray, y_train: np.ndarray, feature_names: Optional[List[str]] = None, tune_hyperparameters: bool = True, 
              find_optimal_k_only: bool = False) -> KNeighborsClassifier:
        """
        Train the KNN model.
    ...
    def detect_outliers(self, X_train: np.ndarray, contamination: float = 0.1) -> Optional[np.ndarray]:
        """
    ...
    def plot_dimensionality_reduction(self, X_train: np.ndarray, y_train: np.ndarray, method: str = 'pca', figsize: Tuple[int, int] = (15, 5)) -> None:
        ...
        if UMAP_AVAILABLE and method == 'umap':
            ...
            if len(self.feature_names) <= 20:  # Only for reasonable number of features
