...
class NeuralNetworkModel:
    ...
    def _train_sklearn(self, X_train: np.ndarray, y_train: np.ndarray, 
                      tune_hyperparameters: bool = False, **kwargs) -> MLPClassifier:
        """
    ...
    def save_model(self, filepath: str) -> None:
        ...
        if self.framework == 'sklearn' and self.model is not None:
            ...
            if filepath.endswith('.pkl'):
                keras_filepath = filepath.replace('.pkl', '.keras')
            elif filepath.endswith('_metadata.joblib'):
                keras_filepath = filepath.replace('_metadata.joblib', '.keras')
            elif filepath.endswith('.joblib'):
                keras_filepath = filepath.replace('.joblib', '.keras')
            else:
            ...
            if filepath.endswith('.pkl'):
                metadata_filepath = filepath.replace('.pkl', '_metadata.joblib')
            elif filepath.endswith('_metadata.joblib'):
                metadata_filepath = filepath  # Already has correct extension
            elif filepath.endswith('.joblib'):
                metadata_filepath = filepath.replace('.joblib', '_metadata.joblib')
            else:
                metadata_filepath = filepath + '_metadata.joblib'
    ...
    def load_model(self, filepath: str) -> None:
        ...
        Args:
            filepath (str): Path to the saved model (for TensorFlow: _metadata.joblib file)
        """
        if self.framework == 'sklearn':
            ...
            # For TensorFlow, filepath should be the _metadata.joblib file
